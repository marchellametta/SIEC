...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 10, 2018 at 04:01 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_tagihan_peserta_lepas` (IN `id_ec` INT)  NO SQL
SELECT peserta_topik.id_peserta, b.biaya_per_topik as tagihan, c.jumlah_topik,IF (SUM(peserta_topik.status_bayar)<COUNT(peserta_topik.id_topik),'true','false') as bayar  from (
    SELECT topik_ec.id_topik,topik_ec.id_ec from topik_ec where topik_ec.id_ec=id_ec
)as a JOIN peserta_topik on peserta_topik.id_topik = a.id_topik JOIN (
    select ec.id_ec, ec.biaya, ec.biaya_per_topik from ec WHERE ec.id_ec=id_ec
) as b on a.id_ec =  b. id_ec JOIN (
    SELECT count(topik_ec.id_topik) as jumlah_topik, topik_ec.id_ec from topik_ec WHERE topik_ec.id_ec=id_ec
) as c on a.id_ec = c.id_ec GROUP BY id_peserta  HAVING COUNT(peserta_topik.id_topik)<c.jumlah_topik$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `get_tagihan_peserta_tetap` (IN `id_ec` INT)  NO SQL
SELECT peserta_topik.id_peserta, b.biaya as tagihan, c.jumlah_topik,IF (SUM(peserta_topik.status_bayar)<COUNT(peserta_topik.id_topik),'true','false') as bayar  from (
    SELECT topik_ec.id_topik,topik_ec.id_ec from topik_ec where topik_ec.id_ec=id_ec
)as a JOIN peserta_topik on peserta_topik.id_topik = a.id_topik JOIN (
    select ec.id_ec, ec.biaya, ec.biaya_per_topik from ec WHERE ec.id_ec=id_ec
) as b on a.id_ec =  b. id_ec JOIN (
...
(22, 1, 1, 1234, 'asdfa', 1, 2, 123123, 'asdfadf', '', 123, 123, NULL, '');
...
(8, '2018-04-10', 'Ruang 9122', '2018-03-10', '2018-03-10', 1, 3),
(9, '2018-04-10', 'Ruang Auditorium', '2018-03-10', '2018-03-10', 1, 4),
...
--
ALTER TABLE `narasumber`
  MODIFY `id_narasumber` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
